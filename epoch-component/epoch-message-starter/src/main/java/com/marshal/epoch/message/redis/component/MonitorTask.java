 ...
 * @auth: Marshal
 * @date: 2020/4/15
 * @desc: 监控队列消息任务, 绑定一个listener, 一个listener绑定一个QueueMonitor,当MonitorTask取到消息后,通过listener去反射调用队列监听器的制定方法
 */
@Slf4j
        ...
        while (running) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.error("exception occurred while get message from queue [" + receiver.getQueue() + "]",
                            thr);
                }
                continue;
            }
