...
/**
 * Redis缓存实现，存储结构使用Hash结构
 *
 * @author Marshal
 * @date 2019/6/26
 */
@Component
public abstract class AbstractRedisCache<T> implements Cache<T>, BeanNameAware {

    private Logger logger = LoggerFactory.getLogger(AbstractRedisCache.class);
    ...
    public void init() {
        ...
        if (StringUtils.isNotEmpty(sqlId)) {
            ...
            for (Object item : resultList) {
                try {
                    redisTemplate.opsForHash().put(getKeyName(),
                            PropertyUtils.getProperty(item, getHashKeyColumn()).toString(), item);
                } catch (Exception e) {
